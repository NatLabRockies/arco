...
use crate::compile::semantic::resolution::{
    resolve_active_model_expressions, resolve_model_scenario_reports,
};
use crate::compile::semantic::sets::{
...
fn validate_projection_source_domains(
    program: &SourceProgram,
    set_registry: &BTreeMap<String, crate::compile::semantic::ResolvedSet>,
    entrypoint: &Path,
...
fn validate_reduce_projection_expressions(
    ...
    for expression in &model.expressions {
        ...
        let arco_kdl::source::ExpressionAbstractionDecl::ReduceProjection {
            projection,
            op,
            target,
        } = abstraction;
...
fn projection_source_keys<'a>(
    projection: &'a arco_kdl::source::ProjectionDecl,
    set_registry: &'a BTreeMap<String, crate::compile::semantic::ResolvedSet>,
    entrypoint: &Path,
...
fn lower_reduce_projection_expression_by_name(
    name: &str,
    expression_abstractions: &BTreeMap<&str, &arco_kdl::source::ExpressionAbstractionDecl>,
    projections: &BTreeMap<&str, &arco_kdl::source::ProjectionDecl>,
    set_registry: &BTreeMap<String, crate::compile::semantic::ResolvedSet>,
    entrypoint: &Path,
    scope: ReduceProjectionLoweringScope,
) -> Result<Option<(String, arco_kdl::algebra::Expr)>, SemanticError> {
...
fn constraint_diagnostic_id(
    constraint: &arco_kdl::source::ConstraintDecl,
    model: &ModelDecl,
    scenario: &ScenarioDecl,
) -> String {
