...
// =============================================================================
// IPOPT facade (gated by `feature = "ipopt"`).
// =============================================================================
...
impl OptimizationAdapter for IpoptArcoAdapter {
    ...
    fn solve(
        ...
        let _ = self.log_to_console;
        Err(ExecutionError::BackendNotAvailable {
            message: "embedded solver family 'ipopt' is not shipped in this build; install or enable the external native IPOPT adapter".to_string(),
        })
    }
}

// ─────────────────────────────────────────────────────────────────────────────
// Public NLP entry point (consumed by the Python bindings when the facade
// feature is enabled). Native IPOPT execution is intentionally outside the
// baseline workspace feature graph so `cargo --workspace --all-features` remains
// portable.
// ─────────────────────────────────────────────────────────────────────────────
...
pub struct NlpVariableSpec {
    ...
    pub upper: f64,
    /// Initial primal value. If `None`, a midpoint or zero is used by native adapters.
...
pub enum NlpError {
    ...
    Infeasible(String),
    #[error(
        "native IPOPT solve execution is not shipped in this build; install or enable the external native IPOPT adapter"
    )]
    Unavailable,
}
...
pub fn solve_nonlinear_problem(
    _nonlinear: &crate::compile::compile::NonlinearProblem,
    _variables: &[NlpVariableSpec],
    _options: &NlpOptions,
) -> Result<NlpSolution, NlpError> {
    Err(NlpError::Unavailable)
}
