...
- `release-please` runs on `main` and maintenance release branches, manages
  release PRs, creates tags and draft GitHub Releases, then calls
  `publish-release.yml` and publishes to PyPI.
- `publish-release.yml` builds CLI binaries and installers, uploads them to
  the GitHub Release, and publishes it.
