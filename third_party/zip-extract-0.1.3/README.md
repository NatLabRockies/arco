...
- `deflate-miniz`: ^ ditto (`miniz` C-backend)
- `deflate-zlib`: ^ ditto (`zlib` C-backend)
...
- `default`: enables `"deflate"` in this Arco vendored patch, because it is
  only used by `scip-sys` to extract SCIP release zip files during builds.
  Disabling unused archive codecs avoids compiling `zstd-sys`, `aes`, and
  `bzip2` on every clean solver build.
